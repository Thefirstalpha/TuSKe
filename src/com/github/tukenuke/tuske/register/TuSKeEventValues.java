...
public class TuSKeEventValues {
	static {
		...
		if (Bukkit.getServer().getPluginManager().isPluginEnabled("Citizens")) {
			Skript.registerEvent("npc Right Click", SimpleEvent.class, NPCRightClickEvent.class,
					"npc right click");
			...
			Classes.registerClass(new ClassInfo<>(NPC.class, "citizen").parser(new Parser<NPC>() {
				@Override
				...
				public NPC parse(String s, ParseContext context) {
					return CitizensAPI.getNPCRegistry().getById(Integer.parseInt(s));
				}
